...
const PACKS = [
  { id: 'p1', name: 'Genesis Premium', description: '8 Cards. Guarantees 1 Epic & 1 Rare. Features Alpha Genesis GX!', cost: 500, cardCount: 8, color: 'from-amber-600 via-yellow-600 to-amber-800', dropRates: { Common: 0.50, Rare: 0.34, Epic: 0.12, Legendary: 0.03, GX: 0.01 }, guaranteed: ['Epic', 'Rare'], set: 'genesis', featuredCardId: 'gx1' },
  { id: 'p2', name: 'Awakening Booster', description: '5 Cards. New elements emerge! Features Eclipse Necromancer GX!', cost: 300, cardCount: 5, color: 'from-blue-700 via-indigo-700 to-slate-900', dropRates: { Common: 0.58, Rare: 0.28, Epic: 0.10, Legendary: 0.03, GX: 0.01 }, guaranteed: ['Rare'], set: 'awakening', featuredCardId: 'a_gx1' },
  { id: 'p3', name: 'Voidfall Booster', description: '6 Cards. Embrace the darkness! Features Chaos Bringer GX!', cost: 400, cardCount: 6, color: 'from-fuchsia-800 via-purple-800 to-slate-950', dropRates: { Common: 0.55, Rare: 0.30, Epic: 0.11, Legendary: 0.03, GX: 0.01 }, guaranteed: ['Rare'], set: 'voidfall', featuredCardId: 'v_gx1' },
  { id: 'p4', name: 'Mythos Booster', description: '5 Cards. Face ancient legends! Features Chronos GX!', cost: 400, cardCount: 5, color: 'from-emerald-700 via-teal-700 to-slate-900', dropRates: { Common: 0.55, Rare: 0.30, Epic: 0.11, Legendary: 0.03, GX: 0.01 }, guaranteed: ['Rare'], set: 'mythos', featuredCardId: 'm_gx1' }
];
...
const AVATARS = [
  "https://api.dicebear.com/9.x/adventurer/svg?seed=Felix", "https://api.dicebear.com/9.x/adventurer/svg?seed=Aneka", "https://api.dicebear.com/9.x/adventurer/svg?seed=Bandit",
  "https://api.dicebear.com/9.x/bottts/svg?seed=Caleb", "https://api.dicebear.com/9.x/bottts/svg?seed=Destiny", "https://api.dicebear.com/9.x/lorelei/svg?seed=Eden",
  "https://api.dicebear.com/9.x/fun-emoji/svg?seed=Fluffy", "https://api.dicebear.com/9.x/adventurer-neutral/svg?seed=Midnight", "https://api.dicebear.com/9.x/avataaars/svg?seed=Jack"
];

const INITIAL_COLLECTION = {};
STARTER_DECK.forEach(id => { INITIAL_COLLECTION[id] = (INITIAL_COLLECTION[id] || 0) + 1; });
...
const openPack = (pack) => {
  ...
  pack.guaranteed.forEach(rarity => {
    let finalRarity = (rarity === 'Legendary' && Math.random() < 0.2) ? 'GX' : rarity;
...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[200] animate-in slide-in-from-top-4 fade-in duration-300">
      <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] border backdrop-blur-xl ${type === 'error' ? 'bg-rose-950/80 border-rose-500/50 text-rose-100' : type === 'success' ? 'bg-emerald-950/80 border-emerald-500/50 text-emerald-100' : 'bg-slate-900/90 border-slate-700 text-slate-200'}`}>
...
const ProfileSetup = ({ onComplete, user, db }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (/[^a-z0-9_]/.test(lowerUsername)) { setError('Only letters, numbers, and underscores allowed.'); return; }
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-4">
            <div className="bg-slate-900 border border-slate-700 rounded-[2rem] p-8 max-w-lg w-full shadow-2xl animate-in zoom-in-95 duration-500 flex flex-col items-center text-center">
                ...
                <div className="w-full mb-8 text-left">
                    ...
                    <input type="text" value={username} onChange={(e) => { setUsername(e.target.value); setError(''); }} placeholder="Enter username" maxLength={16} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-4 text-white outline-none focus:border-blue-500 transition-colors text-center font-bold text-lg tracking-wide" />
          ...
          <div className={`relative w-full h-full rounded-[1.3rem] sm:rounded-[1.6rem] bg-gradient-to-br ${rarityStyle.bg} flex flex-col overflow-hidden border border-white/10 ${card.isMassive ? 'border-yellow-200/50' : ''}`}>
            ...
            <div className={`flex justify-between items-center ${padding} bg-white/40 backdrop-blur-md border-b border-white/20`}>
              ...
              <div className="flex items-center space-x-1 font-black text-rose-700 shrink-0 bg-white/60 px-1.5 py-0.5 rounded-full shadow-sm border border-white/50">
                ...
                <div className={`rounded-full bg-white p-0.5 shadow-sm`}><ElementIcon className={`${elementIconSize} ${elementStyle.color}`} /></div>
              </div>
            </div>
...
const BattleArena = ({ playerDeckIds, onWin, onLose, onExit, difficulty, showToast }) => {
  ...
  useEffect(() => {
    ...
    attempts = 0;
    const poolWeights = { easy: { Common: 85, Rare: 15 }, medium: { Common: 40, Rare: 50, Epic: 10 }, hard: { Rare: 50, Epic: 40, Legendary: 10 }, extreme: { Epic: 30, Legendary: 50, GX: 20 } };
  ...
  useEffect(() => {
    if (gameState === 'botTurn' && bot && player) {
      const executeBotTurn = async () => {
        ...
        let currentBot = { ...bot, deck: [...bot.deck], hand: [...bot.hand], bench: [...bot.bench], active: bot.active ? { ...bot.active } : null };
        let currentPlayer = { ...player, active: player.active ? { ...player.active } : null };
        ...
        if (currentBot.deck.length > 0) {
          ...
          setWinner('player'); setGameState('gameOver'); return;
        }
        ...
        if (currentBot.active && currentPlayer.active && currentBot.active.attachedEnergy > 0) {
           ...
           if (currentPlayer.active.currentHp <= 0) {
              ...
              if (currentBot.prizes <= 0) {
                 setBot({...currentBot}); setPlayer({...currentPlayer});
                 setWinner('bot'); setGameState('gameOver'); return;
              }
           }
           setBot({...currentBot}); setPlayer({...currentPlayer});
        } else {
  ...
  const handleDraw = () => {
    ...
    setPlayer(p => {
      if (p.deck.length === 0) {
        addToLog("You are out of cards! You lose!"); setWinner('bot'); setGameState('gameOver'); return p;
      }
      const newDeck = [...p.deck];
      const drawnCard = newDeck.pop();
      addToLog(`You drew a card.`);
      return { ...p, deck: newDeck, hand: [...p.hand, drawnCard], hasDrawnThisTurn: true };
    });
  };

  const handleHandCardClick = (card, index) => {
    if (gameState !== 'setup' && gameState !== 'playerTurn') return;
    if (selectedHandCard?.index === index) { setSelectedHandCard(null); return; }
  ...
  const handlePlayAreaClick = (area, benchIndex = null) => {
    ...
    if (area === 'active' && !player.active && !card.isEnergy) {
       setPlayer(p => { let newHand = [...p.hand]; newHand.splice(index, 1); return { ...p, hand: newHand, active: card }; });
       ...
       setPlayer(p => { let newHand = [...p.hand]; newHand.splice(index, 1); return { ...p, hand: newHand, bench: [...p.bench, card] }; });
       ...
       if (card.element !== player.active.element) { showToast(`Cannot attach ${card.element} Energy to a ${player.active.element} character!`, 'error'); return; }
       setPlayer(p => { let newHand = [...p.hand]; newHand.splice(index, 1); return { ...p, hand: newHand, active: { ...p.active, attachedEnergy: p.active.attachedEnergy + 1 }, energyAttachedThisTurn: true }; });
       ...
       if (card.element !== player.bench[benchIndex].element) { showToast(`Cannot attach ${card.element} Energy to a ${player.bench[benchIndex].element} character!`, 'error'); return; }
       setPlayer(p => { let newHand = [...p.hand]; newHand.splice(index, 1); let newBench = [...p.bench]; newBench[benchIndex] = { ...newBench[benchIndex], attachedEnergy: newBench[benchIndex].attachedEnergy + 1 }; return { ...p, hand: newHand, bench: newBench, energyAttachedThisTurn: true }; });
  ...
  const handleBenchPromote = (benchIndex) => {
    if (gameState !== 'setup' && gameState !== 'playerTurn') return;
    if (!player.active) {
       setPlayer(p => { let newBench = [...p.bench]; const promoted = newBench.splice(benchIndex, 1)[0]; return { ...p, bench: newBench, active: promoted }; });
  ...
  const handleAttack = () => {
    ...
    if (!player.active) { showToast("You must promote an Active character!", 'error'); return; }
    if (!player.hasDrawnThisTurn) { showToast("You must DRAW a card first! Click your deck.", 'error'); return; }
    if (player.active.attachedEnergy < 1) { showToast("Requires at least 1 Energy attached to attack!", 'error'); return; }
    ...
    if (currentBot.active.currentHp <= 0) {
       ...
       if (player.prizes - prizesToTake <= 0) {
          setBot(currentBot); setWinner('player'); setGameState('gameOver'); return;
       }
    }
    setBot(currentBot);
  ...
  const passTurn = () => {
    if (gameState === 'playerTurn') {
       if (!player.active) { showToast("You must promote an Active character!", 'error'); return; }
       if (!player.hasDrawnThisTurn) { showToast("You must DRAW a card before ending your turn! Click your deck.", 'error'); return; }
  ...
  if (!player || !bot) {
    return <div className="flex-1 flex flex-col items-center justify-center"><Sparkles className="w-16 h-16 text-amber-500 animate-spin" /><p className="text-slate-400 mt-4 font-bold tracking-widest">SHUFFLING DECKS...</p></div>;
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 border border-slate-700 rounded-[2rem] overflow-y-auto shadow-2xl max-h-[85vh] custom-scrollbar backdrop-blur-xl relative">
      ...
      <div className="flex-1 min-h-[280px] shrink-0 bg-slate-950/80 border-b border-slate-800 p-4 flex flex-col relative z-10">
         <div className="absolute top-4 left-4 flex gap-4">
            ...
            <div className="flex flex-col gap-1">
               ...
               <div className="flex gap-1.5">{[...Array(Math.max(0, bot.prizes))].map((_, i) => <div key={i} className="w-6 h-10 bg-amber-600/80 border border-amber-400 rounded shadow-md backface-hidden" style={{ transform: 'rotateY(180deg)' }}></div>)}</div>
            </div>
         </div>
      ...
      <div className="h-16 shrink-0 bg-slate-800/90 backdrop-blur-md flex justify-between items-center px-6 border-y border-slate-700 shadow-[0_0_20px_rgba(0,0,0,0.5)] z-20">
         ...
         <div className="text-slate-300 font-mono text-xs sm:text-sm w-1/3 truncate text-center bg-slate-900/50 px-4 py-2 rounded-lg border border-slate-700">&gt; {log[log.length - 1]}</div>
         <div className="flex space-x-3">
           ...
           <button onClick={passTurn} disabled={gameState !== 'playerTurn' || !player.hasDrawnThisTurn} className="px-5 py-2 bg-slate-700 disabled:bg-slate-800 text-slate-200 font-bold tracking-widest rounded-xl hover:bg-slate-600 transition-all text-xs sm:text-sm border border-slate-500 disabled:border-transparent">PASS</button>
         </div>
      </div>
      {/* PLAYER SIDE */}
      <div className="flex-[1.5] min-h-[420px] shrink-0 bg-slate-900 p-4 flex flex-col justify-between relative z-10">
         ...
         <div className="absolute bottom-6 right-6 flex flex-col items-end">
            ...
            <div className="flex gap-1.5">{[...Array(Math.max(0, player.prizes))].map((_, i) => <div key={i} className="w-8 h-12 bg-amber-600 border border-amber-400 rounded-lg shadow-[0_0_15px_rgba(245,158,11,0.5)]"></div>)}</div>
         </div>
         <div className="flex-1 flex flex-col items-center justify-start mt-4">
            <div className={`w-28 h-40 border rounded-2xl flex items-center justify-center shadow-2xl mb-6 transition-all duration-300 cursor-pointer ${!player.active && selectedHandCard && !selectedHandCard.card.isEnergy ? 'border-amber-400 bg-amber-900/20 shadow-[0_0_30px_rgba(245,158,11,0.2)]' : player.active && selectedHandCard?.card.isEnergy && !player.energyAttachedThisTurn ? 'border-emerald-400 bg-emerald-900/20 shadow-[0_0_30px_rgba(16,185,129,0.2)]' : 'border-slate-700 bg-slate-950/80'}`} onClick={() => handlePlayAreaClick('active')}>
...
const TradeHub = ({ user, db, collection: myCollection, setCollection, showToast, setDbError, profile }) => {
   ...
   const postTrade = async () => {
       ...
       if (!myCollection[offerCardId] || myCollection[offerCardId] <= 0) {
           showToast("You don't own the card you are trying to offer!", 'error'); return;
       }
       setIsPosting(true);
       try {
           setCollection(prev => { const next = { ...prev }; next[offerCardId] -= 1; return next; });
           await addDoc(getTradesCol(db), {
               offererId: user.uid,
               offererName: profile?.username || user.displayName || `Player_${user.uid.substring(0,4)}`,
               offerId: offerCardId, reqId: reqCardId, status: 'open', timestamp: Date.now()
           });
           showToast("Trade posted successfully!", 'success');
           setOfferCardId(''); setReqCardId('');
       } catch(e) {
           console.error(e);
           if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') setDbError(true);
           else showToast("Failed to post trade.", 'error');
       }
       setIsPosting(false);
   };
   ...
   const cancelTrade = async (trade) => {
       try {
           setCollection(prev => { const next = { ...prev }; next[trade.offerId] = (next[trade.offerId] || 0) + 1; return next; });
           await deleteDoc(doc(getTradesCol(db), trade.id));
           showToast("Trade cancelled.", 'info');
       } catch(e) { console.error(e); if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') setDbError(true); else showToast("Cancel failed", 'error'); }
   };

   const acceptTrade = async (trade) => {
       if (!myCollection[trade.reqId] || myCollection[trade.reqId] <= 0) { showToast("You do not own the requested card!", 'error'); return; }
       try {
           setCollection(prev => {
               const next = { ...prev };
               next[trade.reqId] -= 1;
   ...
   return (
      <div className="flex-1 flex flex-col xl:flex-row gap-8 pb-10">
         <div className="w-full xl:w-[450px] shrink-0 bg-slate-900/60 backdrop-blur-xl border border-blue-500/30 rounded-[2rem] p-6 shadow-[0_0_40px_rgba(59,130,246,0.1)] flex flex-col h-fit">
            ...
            <button onClick={postTrade} disabled={!offerCardId || !reqCardId || isPosting} className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-black tracking-[0.2em] rounded-xl shadow-lg transition-all">
...
const OnlineLobby = ({ user, db, profile, onStartMatch, setDbError, showToast }) => {
   ...
   useEffect(() => {
      ...
      const joinLobby = async () => {
         try {
             await setDoc(myRef, { uid: user.uid, name: profile.username, avatar: profile.avatar, status: 'idle', challengerId: null, matchId: null, lastSeen: Date.now() });
         } catch(e) { console.error("Failed to join lobby", e); setDbError(true); }
      };
      joinLobby();
      const interval = setInterval(() => { updateDoc(myRef, { lastSeen: Date.now() }).catch(() => {}); }, 15000);
      ...
      const unsub = onSnapshot(getLobbyCol(db), (snap) => {
          ...
          snap.forEach(d => {
              const data = d.data();
              if (now - data.lastSeen < 45000) activeUsers.push(data);
              if (data.uid === user.uid) setMyStatus(data);
          });
          setLobbyUsers(activeUsers.filter(u => u.uid !== user.uid));
      }, (err) => { console.error(err); if (err.message?.toLowerCase().includes('permission') || err.code === 'permission-denied') setDbError(true); });

      return () => { clearInterval(interval); unsub(); deleteDoc(myRef).catch(() => {}); };
   }, [user, db, profile, setDbError]);

   useEffect(() => {
      if (myStatus && myStatus.matchId) onStartMatch(myStatus.matchId);
   }, [myStatus, onStartMatch]);

   const challengePlayer = async (targetId) => {
      try {
          await updateDoc(doc(getLobbyCol(db), targetId), { challengerId: user.uid, status: 'challenged' });
          await updateDoc(doc(getLobbyCol(db), user.uid), { status: 'waiting_for_accept' });
      } catch(e) { console.error(e); if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') { setDbError(true); } else { showToast("Failed to send challenge", 'error'); } }
   };
   ...
   const acceptChallenge = async () => {
      try {
          ...
          await setDoc(newMatchRef, { hostId: user.uid, guestId: myStatus.challengerId, status: 'waiting_for_guest_deck', turn: user.uid, players: {}, log: ["Match started! Waiting for players..."], winner: null });
          const matchId = newMatchRef.id;
          await updateDoc(doc(getLobbyCol(db), myStatus.challengerId), { matchId: matchId, status: 'playing' });
          await updateDoc(doc(getLobbyCol(db), user.uid), { matchId: matchId, status: 'playing', challengerId: null });
      } catch(e) { console.error(e); if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') { setDbError(true); } else { showToast("Failed to accept challenge", 'error'); } }
   };

   const declineChallenge = async () => {
      try {
          await updateDoc(doc(getLobbyCol(db), user.uid), { challengerId: null, status: 'idle' });
          await updateDoc(doc(getLobbyCol(db), myStatus.challengerId), { status: 'idle' });
   ...
   const cancelMyChallenge = async () => {
       try { await updateDoc(doc(getLobbyCol(db), user.uid), { status: 'idle' }); } catch(e) { console.error(e); if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') setDbError(true); }
   };

   if (!myStatus) return <div className="flex-1 flex items-center justify-center"><Sparkles className="animate-spin text-fuchsia-500 w-12 h-12" /></div>;
       ...
       <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 animate-in fade-in duration-500">
           <div className="w-full max-w-4xl bg-slate-900/80 backdrop-blur-2xl border border-fuchsia-900/50 rounded-[2rem] shadow-2xl overflow-hidden flex flex-col">
               <div className="bg-gradient-to-r from-fuchsia-900/30 to-purple-900/30 border-b border-fuchsia-900/50 p-6 sm:p-8 flex items-center gap-6">
                  <div className="p-4 bg-fuchsia-500/20 rounded-2xl border border-fuchsia-500/30"><Users className="w-8 h-8 sm:w-10 sm:h-10 text-fuchsia-400" /></div>
                  <div><h2 className="text-2xl sm:text-4xl font-black tracking-widest text-white drop-shadow-md">ONLINE LOBBY</h2><p className="text-fuchsia-400 font-bold text-sm sm:text-base tracking-wide">Find an opponent and battle for 1,000 Coins!</p></div>
               </div>
               <div className="p-6 sm:p-8 flex-1 overflow-y-auto min-h-[400px] relative custom-scrollbar">
                   {myStatus.challengerId && myStatus.status === 'challenged' && (
                       <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
                           <div className="bg-slate-900 border border-fuchsia-500/50 rounded-3xl p-8 max-w-md w-full text-center shadow-[0_0_50px_rgba(217,70,239,0.2)] animate-in zoom-in-95">
                               <div className="w-20 h-20 bg-fuchsia-500/20 rounded-full flex items-center justify-center mx-auto mb-6"><Zap className="w-10 h-10 text-fuchsia-400 animate-bounce" /></div>
                   ...
                   {myStatus.status === 'waiting_for_accept' && (
                       <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
                           <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-md w-full text-center shadow-xl animate-in zoom-in-95">
                               <Sparkles className="w-12 h-12 text-fuchsia-500 mx-auto mb-6 animate-spin" /><h3 className="text-xl font-black text-white mb-8 tracking-widest">WAITING FOR OPPONENT...</h3>
                   ...
                   <div className="space-y-4">
                       {lobbyUsers.length === 0 ? (
                           <div className="text-center py-24 opacity-50 flex flex-col items-center"><Ghost className="w-16 h-16 mb-4 text-slate-500" /><p className="font-bold tracking-widest text-slate-400">NO OTHER PLAYERS ONLINE</p></div>
                       ) : (
                           lobbyUsers.map(u => (
                                   ...
                                   <div className="flex items-center gap-5">
                                       ...
                                       <div>
                                           ...
                                           <div className="flex items-center gap-2 mt-1">
                                             ...
                                             <span className="text-xs font-black tracking-widest text-slate-400 uppercase">{u.status === 'idle' ? 'READY TO BATTLE' : 'IN MENU'}</span>
                                           </div>
                                       </div>
                                   </div>
                                   <button onClick={() => challengePlayer(u.uid)} disabled={u.status !== 'idle'} className="px-6 py-3 bg-fuchsia-600/10 text-fuchsia-400 border border-fuchsia-500/30 rounded-xl font-black tracking-widest hover:bg-fuchsia-600 hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:shadow-[0_0_20px_rgba(217,70,239,0.4)]">CHALLENGE</button>
                               </div>
                           ))
                       )}
                   </div>
...
const OnlineBattleArena = ({ playerDeckIds, onWin, onLose, onExit, user, db, existingMatchId, showToast, setDbError }) => {
  ...
  useEffect(() => {
       ...
       try {
           ...
           if (snap.exists()) {
               ...
               if (data.guestId === user.uid && (!data.players || !data.players[user.uid])) {
                   await updateDoc(matchRef, { [`players.${user.uid}`]: initializePlayerState(playerDeckIds), status: 'playing' });
               } else if (data.hostId === user.uid && (!data.players || !data.players[user.uid])) {
                   await updateDoc(matchRef, { [`players.${user.uid}`]: initializePlayerState(playerDeckIds) });
               }
           }
       } catch (e) { console.error(e); if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') setDbError(true); }
    };
    ...
    const unsub = onSnapshot(doc(getMatchesCol(db), existingMatchId), (snap) => {
        if (snap.exists()) { setMatchData(snap.data()); } else { showToast("Match ended abruptly.", 'error'); onExit(); }
    }, (err) => { console.error("Arena Snapshot Error", err); if (err.message?.toLowerCase().includes('permission') || err.code === 'permission-denied') setDbError(true); });
    return () => unsub();
  }, [existingMatchId, db, user, playerDeckIds, onExit, showToast, setDbError]);

  const updateMatch = async (updates) => {
     try { await updateDoc(doc(getMatchesCol(db), existingMatchId), updates); } catch (e) { console.error("Sync failed", e); if (e.message?.toLowerCase().includes('permission') || e.code === 'permission-denied') setDbError(true); }
  };

  if (!matchData || !matchData.players || !matchData.players[user.uid]) return <div className="flex-1 flex flex-col items-center justify-center gap-6"><Sparkles className="w-16 h-16 text-fuchsia-500 animate-spin" /><h2 className="text-3xl font-black tracking-widest text-white animate-pulse">CONNECTING TO ARENA...</h2></div>;
  ...
  if (!opponent) return <div className="flex-1 flex flex-col items-center justify-center gap-6"><Zap className="w-16 h-16 text-fuchsia-500 animate-bounce" /><h2 className="text-2xl font-black tracking-widest text-white">WAITING FOR OPPONENT'S DECK...</h2></div>;
  ...
  const handleDraw = async () => {
    ...
    if (me.deck.length === 0) { await updateMatch({ winner: opponentId, status: 'gameover', log: [...matchData.log, "Player ran out of cards!"].slice(-10) }); return; }
    const newDeck = [...me.deck];
    const drawnCard = newDeck.pop();
    await updateMatch({ [`players.${user.uid}.deck`]: newDeck, [`players.${user.uid}.hand`]: [...me.hand, drawnCard], [`players.${user.uid}.hasDrawnThisTurn`]: true, log: [...matchData.log, "Opponent drew a card."].slice(-10) });
  };
  ...
  const handlePlayAreaClick = async (area, benchIndex = null) => {
    ...
    if (area === 'active' && !me.active && !card.isEnergy) {
       const newHand = [...me.hand]; newHand.splice(index, 1);
       await updateMatch({ [`players.${user.uid}.hand`]: newHand, [`players.${user.uid}.active`]: card, log: [...matchData.log, `Opponent played ${card.name} to Active.`].slice(-10) });
       setSelectedHandCard(null);
    } 
    else if (area === 'bench' && me.bench.length < 5 && !card.isEnergy) {
       const newHand = [...me.hand]; newHand.splice(index, 1);
       await updateMatch({ [`players.${user.uid}.hand`]: newHand, [`players.${user.uid}.bench`]: [...me.bench, card], log: [...matchData.log, `Opponent played ${card.name} to Bench.`].slice(-10) });
       setSelectedHandCard(null);
    }
    else if (area === 'active' && card.isEnergy && me.active && !me.energyAttachedThisTurn && isMyTurn) {
       if (card.element !== me.active.element) { showToast(`Cannot attach ${card.element} Energy to a ${me.active.element} character!`, 'error'); return; }
       const newHand = [...me.hand]; newHand.splice(index, 1);
       await updateMatch({ [`players.${user.uid}.hand`]: newHand, [`players.${user.uid}.active.attachedEnergy`]: me.active.attachedEnergy + 1, [`players.${user.uid}.energyAttachedThisTurn`]: true, log: [...matchData.log, `Opponent attached Energy.`].slice(-10) });
       setSelectedHandCard(null);
    }
    else if (area === 'benchCard' && card.isEnergy && benchIndex !== null && !me.energyAttachedThisTurn && isMyTurn) {
       if (card.element !== me.bench[benchIndex].element) { showToast(`Cannot attach ${card.element} Energy to a ${me.bench[benchIndex].element} character!`, 'error'); return; }
       const newHand = [...me.hand]; newHand.splice(index, 1);
       const newBench = [...me.bench]; newBench[benchIndex].attachedEnergy += 1;
       await updateMatch({ [`players.${user.uid}.hand`]: newHand, [`players.${user.uid}.bench`]: newBench, [`players.${user.uid}.energyAttachedThisTurn`]: true, log: [...matchData.log, `Opponent attached Energy to bench.`].slice(-10) });
       setSelectedHandCard(null);
    }
  };
  ...
  const handleBenchPromote = async (benchIndex) => {
    ...
    await updateMatch({ [`players.${user.uid}.bench`]: newBench, [`players.${user.uid}.active`]: promoted, log: [...matchData.log, `Opponent promoted ${promoted.name} to Active.`].slice(-10) });
  };
  ...
  const handleAttack = async () => {
    ...
    let updates = { [`players.${opponentId}.active.currentHp`]: newOppHp, turn: opponentId, [`players.${user.uid}.energyAttachedThisTurn`]: false, [`players.${user.uid}.hasDrawnThisTurn`]: false };
    ...
    if (newOppHp <= 0) {
       ...
       updates[`players.${opponentId}.active`] = null; updates[`players.${user.uid}.prizes`] = newPrizes;
       if (newPrizes <= 0) { updates.winner = user.uid; updates.status = 'gameover'; }
    }
  ...
  const passTurn = async () => {
    ...
    await updateMatch({ turn: opponentId, [`players.${user.uid}.energyAttachedThisTurn`]: false, [`players.${user.uid}.hasDrawnThisTurn`]: false, log: [...matchData.log, "Opponent passed their turn."].slice(-10) });
  };

  const leaveMatch = async () => {
     if (matchData.status === 'playing') { await updateMatch({ winner: opponentId, status: 'gameover' }); } else { deleteDoc(doc(getMatchesCol(db), existingMatchId)).catch(()=>{}); }
  ...
  if (matchData.status === 'gameover') {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in-95 duration-500">
         <h2 className={`text-6xl sm:text-8xl font-black mb-8 tracking-[0.3em] drop-shadow-2xl ${isWinner ? 'text-fuchsia-400' : 'text-slate-500'}`}>{isWinner ? 'ONLINE VICTORY' : 'DEFEAT'}</h2>
         <p className="text-xl sm:text-2xl text-slate-300 mb-12 font-medium tracking-wide">{isWinner ? 'You proved your dominance! +1000 Coins' : 'You were outmatched. +100 Coins'}</p>
         <button onClick={() => { isWinner ? onWin() : onLose(); leaveMatch(); }} className="px-12 py-4 bg-fuchsia-600 text-white font-black tracking-widest rounded-full hover:bg-fuchsia-500 transition-all hover:scale-105 shadow-[0_10px_30px_rgba(192,38,211,0.4)]">COLLECT REWARD</button>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col bg-slate-900 border border-slate-700 rounded-[2rem] overflow-y-auto shadow-2xl max-h-[85vh] custom-scrollbar backdrop-blur-xl relative">
      ...
      <div className="flex-1 min-h-[280px] shrink-0 bg-slate-950/80 border-b border-slate-800 p-4 flex flex-col relative z-10">
         <div className="absolute top-4 left-4 flex gap-4">
            ...
            <div className="flex flex-col gap-1">
               <span className="text-slate-500 font-bold text-[0.6rem] uppercase tracking-widest">Prizes</span>
               <div className="flex gap-1.5">{[...Array(Math.max(0, opponent?.prizes || 0))].map((_, i) => <div key={i} className="w-6 h-10 bg-fuchsia-600/80 border border-fuchsia-500 rounded shadow-md backface-hidden"></div>)}</div>
            </div>
         </div>
         <div className="absolute top-4 right-4 flex gap-1.5">
         ...
         <div className="flex-1 flex flex-col items-center justify-center mt-8">
            <div className="flex gap-3 mb-6 h-24">
               {[...Array(5)].map((_, i) => (
                 <div key={i} className="w-16 h-24 border border-slate-700 rounded-xl flex items-center justify-center bg-slate-900/50 shadow-inner">{opponent?.bench?.[i] && <TCGCard card={opponent.bench[i]} size="mini" inBattle={true} />}</div>
               ))}
            </div>
      ...
      <div className="h-16 shrink-0 bg-slate-800/90 backdrop-blur-md flex justify-between items-center px-6 border-y border-slate-700 shadow-[0_0_20px_rgba(0,0,0,0.5)] z-20">
         ...
         <div className="text-slate-300 font-mono text-xs sm:text-sm w-1/3 truncate text-center bg-slate-900/50 px-4 py-2 rounded-lg border border-slate-700">&gt; {matchData.log[matchData.log.length - 1]}</div>
         <div className="flex space-x-3">
      ...
      <div className="flex-[1.5] min-h-[420px] shrink-0 bg-slate-900 p-4 flex flex-col justify-between relative z-10">
         ...
         <div className="absolute bottom-6 right-6 flex flex-col items-end">
            ...
            <div className="flex gap-1.5">{[...Array(Math.max(0, me.prizes))].map((_, i) => <div key={i} className="w-8 h-12 bg-fuchsia-600 border border-fuchsia-400 rounded-lg shadow-[0_0_15px_rgba(217,70,239,0.5)]"></div>)}</div>
         </div>
         <div className="flex-1 flex flex-col items-center justify-start mt-4">
            <div className={`w-28 h-40 border rounded-2xl flex items-center justify-center shadow-2xl mb-6 transition-all duration-300 cursor-pointer ${!me.active && selectedHandCard && !selectedHandCard.card.isEnergy ? 'border-fuchsia-400 bg-fuchsia-900/20 shadow-[0_0_30px_rgba(217,70,239,0.2)]' : me.active && selectedHandCard?.card.isEnergy && !me.energyAttachedThisTurn ? 'border-emerald-400 bg-emerald-900/20 shadow-[0_0_30px_rgba(16,185,129,0.2)]' : 'border-slate-700 bg-slate-950/80'}`} onClick={() => handlePlayAreaClick('active')}>
            ...
            <div className="flex gap-3 h-24">
               {[...Array(5)].map((_, i) => (
                 <div key={i} className={`w-16 h-24 border rounded-xl flex items-center justify-center transition-all duration-300 cursor-pointer ${!me.bench[i] && selectedHandCard && !selectedHandCard.card.isEnergy ? 'border-fuchsia-400/50 bg-fuchsia-900/10' : me.bench[i] && selectedHandCard?.card.isEnergy && !me.energyAttachedThisTurn ? 'border-emerald-400 bg-emerald-900/20' : 'border-slate-800 bg-slate-900/50'}`} onClick={() => me.bench[i] && selectedHandCard?.card.isEnergy ? handlePlayAreaClick('benchCard', i) : me.bench[i] ? handleBenchPromote(i) : handlePlayAreaClick('bench')}>
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      const myFirebaseConfig = {
        apiKey: "AIzaSyCcNMYKx4XMQnELN1Lgx8RMbYX3bUeFqd8", authDomain: "mythic-pulls.firebaseapp.com",
        projectId: "mythic-pulls", storageBucket: "mythic-pulls.firebasestorage.app",
        messagingSenderId: "127448359944", appId: "1:127448359944:web:790aaa8de8928241e6ddbd",
      ...
      const initAuth = async () => {
        ...
        else if (isGitHub) setShowLogin(true);
        else await signInAnonymously(auth);
      };
      initAuth();
      const unsubscribe = onAuthStateChanged(auth, u => {
         if (u) { setUser(u); setShowLogin(false); } else if (isGitHub) { setShowLogin(true); }
      });
      return () => unsubscribe();
  ...
  const handleGoogleLogin = async () => {
     ...
     catch (e) { console.error(e); showToast("Login failed! " + e.message, 'error'); }
  };

  const handleLogout = () => { signOut(getAuth()); setShowLogin(true); };

  useEffect(() => {
    if (!user || !db) return;
    const loadData = async () => {
       try {
          const snap = await getDoc(getSaveDocRef(db, user.uid));
          if (snap.exists()) {
             const data = snap.data();
  ...
  const startUnboxing = (pulledCards, itemObj, type) => {
      setCurrentCards(pulledCards); setActiveCardIndex(0); setIsCardRevealed(false); setShowSummary(false);
      setActivePackName(itemObj.name); setOpeningItem({ ...itemObj, type }); setIsUnboxing(true);
      setUnboxStage('shake'); setActiveTab('opening');
      setTimeout(() => { setUnboxStage('burst'); setTimeout(() => setIsUnboxing(false), 600); }, 1500); 
  };
  ...
  const handleCardInteraction = () => {
    if (!isCardRevealed) setIsCardRevealed(true);
    else if (activeCardIndex < currentCards.length - 1) {
        setIsCardRevealed(false); setTimeout(() => setActiveCardIndex(prev => prev + 1), 50);
    } else setShowSummary(true);
  };

  const handleRevealAll = () => { setIsCardRevealed(true); setShowSummary(true); };

  const MAX_DECK_SIZE = 30; 
  const addToDeck = (cardId) => {
    const ownedCount = collection[cardId] || 0;
    if (deck.filter(id => id === cardId).length < ownedCount && deck.length < MAX_DECK_SIZE) setDeck(prev => [...prev, cardId]);
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-amber-500/30 overflow-x-hidden relative flex flex-col">
      ...
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl bg-slate-900/70 backdrop-blur-2xl border border-slate-700/50 p-2 sm:p-3 rounded-full z-50 shadow-[0_10px_40px_rgba(0,0,0,0.5)] flex justify-between items-center">
        <div className="flex items-center space-x-3 pl-4 sm:pl-6">
          ...
          <h1 className="text-lg sm:text-2xl font-black text-white tracking-[0.2em] hidden lg:block drop-shadow-md">MYTHIC</h1>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 bg-slate-950 px-3 sm:px-4 py-1 sm:py-2 rounded-full border border-slate-800 shadow-inner mr-1 sm:mr-2">
          <button onClick={() => setShowRules(true)} className="text-slate-400 hover:text-slate-200 transition-colors hidden lg:block mr-2" title="How to Play"><BookOpen className="w-5 h-5" /></button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
                 ...
                 <button onClick={() => { 
                         ...
                         CHARACTERS.forEach(c => {
                             ...
                             if (['Rare', 'Epic', 'Legendary', 'GX'].includes(c.rarity)) newCol[c.id + '_massive'] = (newCol[c.id + '_massive'] || 0) + 10;
                         });
